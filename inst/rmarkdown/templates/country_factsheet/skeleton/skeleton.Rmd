...
if (packageVersion("unhcrdatapackage") != "0.1.9") {
...
# Data preparation using refugees package

pop_long <- refugees::population |>
  dplyr::filter(
    year == params$year | year == as.numeric(params$year) - 1,
    coa_iso == params$country
  ) |>
  dplyr::select(year, coa_iso, coo_name, refugees, asylum_seekers, idps, oip, stateless, others_of_concern) |>
  tidyr::pivot_longer(
    cols = c(refugees, asylum_seekers, idps, oip, stateless, others_of_concern),
    names_to = "Population.type",
    values_to = "Value"
  ) |>
  dplyr::mutate(Population.type = dplyr::case_when(
    Population.type == "refugees" ~ "REF",
    Population.type == "asylum_seekers" ~ "ASY",
    Population.type == "idps" ~ "IDP",
    Population.type == "oip" ~ "OIP",
    Population.type == "stateless" ~ "STA",
    Population.type == "others_of_concern" ~ "OOC",
    TRUE ~ Population.type
  ))
