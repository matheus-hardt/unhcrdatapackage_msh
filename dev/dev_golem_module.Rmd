        ...
        fluidRow(
          ...
          column(
            ...
            if (thisPlot %in% c("ctr_processing_time")
              ...
              selectizeInput(
                inputId = ns("country_origin_iso3c"),
            ...
            if (thisPlot %in% c(
              ...
              selectInput(
                ...
                selected = "RefugeeRecognitionRate"
              )
            } else {
              ""
            },
            ...
            if (thisPlot %in% c(
              ...
              selectInput(
                ...
                label = "Order by",
                choices = c(
                  "Recognized Refugee Status Decisions" = "Recognized",
...
mod_plotviz_server <- function(id, thisPlot, reactiveParameters) {
  moduleServer(id, function(input, output, session) {
    ...
    output$annotinfo2 <- shiny::renderText({
      # browser()
      paste0(
        "A long brush click to draw the \n  box where the annotation \n should be overlaid: ",
    ...
    output$dl <- downloadHandler(
      filename = function() {
        paste(thisPlot, "_",
          ...
          format(Sys.time(), "%Y_%m_%d_%H_%M_%S"), ".", input$format,
          sep = ""
        )
      },
...
mod_categories_ui <- function(id) {
  ...
  tagList(
    tabsetPanel(
      type = "tabs",
      tabPanel(
        title = "Population Type Over Year",
